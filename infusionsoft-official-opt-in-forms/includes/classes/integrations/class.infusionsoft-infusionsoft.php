...
class infusionsoft_infusionsoft extends INF_Infusionsoft {
	...
	public function draw_infusionsoft_form( $form_fields, $service, $field_values ) {
		$form_fields .= sprintf( '
			...
			__( 'PAT or SAK API Key', 'infusionsoft' ), __( 'Application name', 'infusionsoft' ),
	...
	function subscribe_infusionsoft( $api_key, $app_id, $email, $name = '', $last_name = '', $optin_uniqid ) {
		...
		try {
            ...
            if ($e->getCode() === 16) {
                $error_message = "The email provided has exercised the right to be forgotten. Please provide a different email.";
            } else {
                $error_message = $e->getMessage();
            }
        }
